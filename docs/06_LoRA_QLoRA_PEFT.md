...
2. [PEFT 总览与 LoRA 核心思想](#peft-总览与-lora-核心思想)
3. [LoRA 矩阵维度与参数量计算](#lora-矩阵维度与参数量计算)
4. [B 初始化为 0 的原理](#b-初始化为-0-的原理)
5. [LoRA 加在哪些层](#lora-加在哪些层)
6. [Attention vs MLP 的选择](#attention-vs-mlp-的选择)
7. [Rank 怎么选：医疗 SFT 场景分析](#rank-怎么选医疗-sft-场景分析)
8. [QLoRA 深度解析](#qlora-深度解析)
9. [4bit 训练 vs 4bit 部署](#4bit-训练-vs-4bit-部署)
10. [merge_and_unload 的底层逻辑](#merge_and_unload-的底层逻辑)
11. [合并后的文件清单](#合并后的文件清单)
12. [safetensors / adapter_config / adapter_model](#safetensors--adapter_config--adapter_model)
13. [如何验证 LoRA 合并成功](#如何验证-lora-合并成功)
14. [DPO 继续从 SFT LoRA checkpoint 训练的注意事项](#dpo-继续从-sft-lora-checkpoint-训练的注意事项)
15. [单卡 RTX 5090 为什么适合 QLoRA](#单卡-rtx-5090-为什么适合-qlora)
16. [代码实现示例](#代码实现示例)
17. [面试追问与回答策略](#面试追问与回答策略)
18. [背诵版总结](#背诵版总结)
...
LoRA 的核心思想来自一个经验观察：**预训练大模型在适配下游任务时，权重更新矩阵 ΔW 是低秩的**。
...
以 Qwen3-8B 的一个 attention 线性层为例（假设 hidden_size=4096, r=16）：
...
其中：
- A 矩阵：d × r = 4096 × 16 = 65,536 参数
- B 矩阵：r × k = 16 × 4096 = 65,536 参数
- 总计约 0.13M / 模块

**全模型 LoRA 参数量（项目的 target_modules 配置）**：

在 Qwen3-8B 中，总参数量约 8B，每个 Transformer 层有 7 个 target linear（q_proj、k_proj、v_proj、o_proj、gate_proj、up_proj、down_proj），共 32~36 层。

```
一个 LoRA 模块: ~0.13M 参数
每层 7 个模块: ~0.9M 参数
32 层总计:     ~29M 参数（所有 q/k/v/o + MLP）
36 层总计:     ~33M 参数

但实际项目中有时 gate/up 采用不同的 intermediate_size，需具体分析。
以项目 Qwen3-8B 为例，实际可训练参数约 5.4M —— 占总参数 0.07%。
```

**为什么实际只有约 5.4M 而非 30M？**
因为 Qwen3 的 attention 使用了 GQA（Grouped Query Attention），k_proj 和 v_proj 的维度可能小于 q_proj。同时 v_proj 的 hidden_size 也不同。实际总参数量需要逐个模块累加。

### rank、alpha、dropout、scaling 的关系

| 超参数 | 含义 | 项目取值 | 面试话术 |
|--------|------|----------|----------|
| `r` (rank) | 低秩维度，决定 adapter 的表达能力 | 16 | "rank 越大表达能力越强但参数越多，医学任务需要一定容量来记住术语和诊疗逻辑" |
| `alpha` | 缩放因子，控制 adapter 输出的幅度 | 32 | "alpha=32, r=16 时 scaling=2，让 LoRA 输出的梯度与原始权重梯度在同一数量级" |
| `lora_dropout` | adapter 层的 dropout，正则化 | 0.05 | "5% dropout 适度防止过拟合，医学数据量通常不大所以需要" |
| `scaling` | α/r，实际应用于 BAx 前的系数 | 2.0 | "scaling 让不同 rank 的超参互相独立，调 rank 不调 lr" |
...
# 其中 self.scaling = self.lora_alpha[adapter] / self.r[adapter]
```

**关键点**：scaling 放在 B 之后还是 A 之后本质等价（线性变换的可交换性），但 PEFT 实现中放在 B·A 整体之后。
...
> "B 初始化为零保证训练初始时刻 LoRA adapter 的输出为零，模型行为等于预训练模型，不会引入随机扰动。同时 A 的随机初始化确保反向传播时 B 有非零梯度，即训练的起点是基座模型的能力水平，终点是任务适配能力，中间是平滑过渡。"
...
## Attention vs MLP 的选择

### 只加 Attention（保守方案）

**优点**：
- 参数量更少（4 个模块 vs 7 个模块，每层）
- 训练更快、显存更省
- 适合只需要调整"注意力模式"的任务，如对话风格、格式遵循

**缺点**：
- MLP 存储了大量事实性/世界知识，不调 MLP 可能导致新领域知识注入不足
- 对于需要记住新术语、新概念的任务（如医学术语），效果打折

### 加 Attention + MLP（激进方案，项目采用）

**优点**：
- 覆盖模型全部前向路径，表达能力最强
- MLP 的 gate_proj/up_proj/down_proj 是知识存储的关键层，在医学领域注入疾病、药物、诊疗逻辑等知识时不可忽略
- 实际经验表明，医疗 SFT 中不加 MLP 的 LoRA 在事实性问答上准确率明显下降
...
> "在对话式任务或格式调整中只加 attention 就够了，但在医学微调中，大量新术语、药物名称、诊断标准需要被模型'记住'——这些事实性信息主要存储在 MLP 的前馈权重中。所以我选择对 attention 和 MLP 都加 LoRA，虽然参数量多了约 75%，但代价可控（QLoRA 下仅增加几百 MB 显存），换来了更完整的知识注入能力。"

---

## Rank 怎么选：医疗 SFT 场景分析
...
> "r 的选择是表达能力和泛化性的权衡。医学 SFT 需要模型学习大量专业术语和诊疗逻辑——这些信息密度远高于通用对话，所以不能用 r=4。同时项目有 ~10K 条数据，r=64 可能过拟合。r=16 配合 alpha=32（scaling=2）在实验中表现稳定，repetition 和事实准确率都有保障。如果后续有更多数据，可以考虑升到 r=32。"
...
```
传统量化：将 FP32/FP16 权重均匀或按百分位映射到 4-bit（16 个离散值）
NF4 量化：假设权重服从零均值正态分布，在正态分布的高概率区域分配更多量化值，
         在尾部区域分配更少量化值，信息论上最优。
...
```
第一重量化：FP32 → 4bit NF4（每个 4bit 权重需要 1 个 32bit 量化常数）
第二重量化：32bit 量化常数 → 8bit FP8（进一步压缩量化常数的存储）

效果：每个 4bit 参数额外节省约 0.373 bit 的存储开销
      在 8B 模型上节省约 0.5GB 显存
```

#### 3. Paged Optimizer（分页优化器）

```
传统优化器：所有优化器状态（如 AdamW 的 m 和 v）都在 GPU 显存中
Paged Optimizer：当 GPU 显存不足时，将优化器状态暂时移到 CPU RAM（统一内存），
                 需要时再换回 GPU。避免了 OOM 中断训练。

本质是 CUDA 的统一虚拟寻址（UVA）—— GPU 和 CPU 共享地址空间，
显存不够时自动 spill 到 CPU RAM。
```

#### 4. BF16 Compute Dtype（BF16 计算精度）

```
存储格式：权重存为 4bit NF4，量化常数存为 8bit
计算格式：LoRA 权重和中间激活使用 BF16

前向传播时：
  1. 将 4bit NF4 反量化为 BF16
  2. 在 BF16 精度下计算 forward
  3. 反向传播在 BF16 下计算梯度
  4. 只更新 LoRA adapter（始终在 BF16）
```

BF16 相比 FP16 的优势：指数位更多（8 vs 5），动态范围更大（约 10^38 vs 10^4），不会出现 FP16 常见的梯度上溢问题。
...
> "QLoRA 的 4bit 是为了训练——冻结的 base model 权重压缩到 4bit 减少显存，但计算仍在 BF16；训练完 merge_and_unload 后得到的是 BF16 精度的完整模型。推理时的 4bit 部署（如 GPTQ/AWQ）是为了降低推理延迟和显存——权重永久量化到 INT4 且直接在这上面计算。两者目标不同、量化方法不同、前后处理不同。我们项目训练用 QLoRA，部署用合并后的 BF16 模型——因为医疗场景对精度要求极高，不敢在推理侧做 4bit 量化造成信息损失。"

---

## merge_and_unload 的底层逻辑
...
1. **计算 ΔW = B·A**：将 A(d×r) 和 B(r×k) 矩阵相乘得到 ΔW(d×k)，与原始 W 同维度
2. **缩放**：乘以 scaling = α/r
3. **矩阵加法**：W_base + scaled ΔW
4. **替换**：将层的 weight 从原始 W 替换为 W_merged
5. **删除 adapter 层**：移除 LoRA 的前向 hook/forward 方法，恢复为标准 nn.Linear

### 合并后的权重精度

```
训练时：
  Base weight:    4bit NF4（存储）→ 反量化为 BF16（计算）
  LoRA adapter:   BF16
  合并后的 W:     BF16（完整精度）

合并后保存的是 BF16 或 FP32 权重，不再有量化信息。
这意味着合并后的模型和用全量 BF16 SFT 得到的模型在格式上是完全一致的。
```
...
当执行 `model.merge_and_unload()` 后，保存的是标准 HuggingFace transformer 模型：
...
**注意**：合并后的模型**不包含**以下文件：
- `adapter_config.json` — 因为 adapter 已被合并，不再是 PEFT 格式
- `adapter_model.safetensors` — 同上

---

## safetensors / adapter_config / adapter_model

### safetensors 是什么

```
.safetensors 是 HuggingFace 开发的张量序列化格式，替代 .bin / .pt

优势：
  1. 安全：不执行任意 Python 代码，避免 pickle 安全漏洞
  2. 快速：支持零拷贝加载（内存映射 mmap）
  3. 懒加载：只加载实际用到的 tensor（如多分片场景中按 index 加载）
  4. 跨框架：PyTorch、TensorFlow、JAX 都可读写

格式：
  [header_size: 8 bytes][JSON header: N bytes][tensor data: M bytes]
  每个 tensor 名和 offset 在 JSON header 中，数据在二进制块中
```

### adapter_config.json 是什么

PEFT adapter 的元数据配置文件，记录 LoRA 的所有超参数：
...
**用途**：
- 加载 adapter 时，PEFT 库根据此配置重建 LoRA 模块结构
- 合并时，根据此配置知道哪些层需要合并
- 迁移 adapter 到不同 base model 时需要匹配

### adapter_model.safetensors 是什么

```
adapter_model.safetensors 只包含 LoRA adapter 的权重张量：

内容示例（Qwen3-8B, r=16, 7 target modules, 32 layers）：
  ...
  - base_model.model.model.layers.0.self_attn.k_proj.lora_A.weight: [16, 1024]
  - ... (每层每个 target module 各一个 A 和 B)
  - base_model.model.model.layers.31.mlp.down_proj.lora_B.weight: [4096, 16]

文件大小估算：
  Qwen3-8B total params ≈ 7.6B
  其中 target_modules 覆盖了约 65-75% 的前向路径
  以 70% 计，LoRA 参数 ≈ 7.6B × 0.7 × (2r / d)
  对于 r=16, d=4096: ≈ 7.6B × 0.7 × (32/4096) ≈ 40M 参数
  校验：40M × 2 bytes (BF16) ≈ 80 MB（但实际可能更少，因为 k_proj/v_proj 维度小）
```
...
for name, param in base_model.named_parameters():
    ...
    diff = (merged_param - param).abs().max().item()
    if diff == 0:
        print(f"[UNCHANGED] {name}")
    else:
        print(f"[MERGED] {name}, max_diff={diff:.6f}")
```

### 方法二：推理验证法

```python
# 用同一个 prompt 分别在三种模式下推理：
# 1. base_model (no adapter)
# 2. peft_model (with adapter, not merged)
# 3. merged_model (merged)

input_ids = tokenizer("什么是高血压？", return_tensors="pt").input_ids

output_base = base_model.generate(input_ids, max_new_tokens=50)
output_peft = peft_model.generate(input_ids, max_new_tokens=50)
output_merged = merged_model.generate(input_ids, max_new_tokens=50)

# output_peft 和 output_merged 应该完全一致
assert torch.equal(output_peft, output_merged), "合并后推理结果不一致！"
```
...
W_base = base_model.get_parameter(f"{module_name}.weight")
lora_A = peft_model.get_parameter(f"base_model.model.{module_name}.lora_A.default.weight")
lora_B = peft_model.get_parameter(f"base_model.model.{module_name}.lora_B.default.weight")
scaling = peft_model.peft_config["default"].lora_alpha / peft_model.peft_config["default"].r

W_expected = W_base + scaling * (lora_B @ lora_A)
W_actual = merged_model.get_parameter(f"{module_name}.weight")

assert torch.allclose(W_expected, W_actual, atol=1e-5), "合并计算不正确！"
```

### 面试话术

> "验证 LoRA 合并是否成功有三种方法：一是逐参数比较合并前后的差异（理论上只有 target_modules 有变化），二是用同一输入对比 peft_model.generate() 和 merged_model.generate() 的输出应完全一致，三是手动计算 W_merged = W_base + scaling * B @ A 并与实际合并后的权重对比。项目中我通常在合并后用第三种方法抽样验证前几层。"

---

## DPO 继续从 SFT LoRA checkpoint 训练的注意事项

### 1. 加载 SFT adapter 而非 base model
...
# 此时 model 是 base model + SFT LoRA adapter（adapter 可训练）

# 注意：如果调用了 merge_and_unload()，则无法继续 LoRA 训练
# 必须保持 peft model 格式（base frozen + adapter trainable）
```

### 2. DPO 的 reference model 加载

```python
# reference model 应该是 SFT 模型（冻结），policy model 在此基础上训练
ref_model = AutoModelForCausalLM.from_pretrained(base_model_path, **qlora_kwargs)
ref_model = PeftModel.from_pretrained(ref_model, sft_lora_path)  # 加载 SFT adapter
ref_model = ref_model.merge_and_unload()  # 可选：合并后冻结，省显存

# 或者：ref_model 保持 QLoRA 格式但 requires_grad=False
```

### 3. 不要重新初始化 LoRA

```python
# 错误：在 SFT checkpoint 上又加一新 LoRA
model = PeftModel.from_pretrained(model, sft_lora_path)
model.add_adapter("dpo", lora_config)  # ❌ 堆叠两层 LoRA，不是继续训练

# 正确：直接加载 SFT adapter 并设为可训练
model = PeftModel.from_pretrained(model, sft_lora_path, is_trainable=True)
# ✅ 在 SFT adapter 的基础上继续更新
```

### 4. 学习率与 beta 调整

```
SFT 阶段 lr 通常较高（如 2e-4），因为是从零开始学 LoRA
DPO 阶段 lr 应降低（如 5e-5 或 1e-5），因为 adapter 已有基础
          并且 DPO 的 loss landscape 更陡峭
beta 通常取 0.1~0.5，控制与 reference model 的 KL 散度约束强度
```

### 5. 保存 DPO checkpoint

```python
# DPO 训练结束后保存新的 adapter
trainer.save_model("./dpo_lora_checkpoint")

# 保存的内容：
# ./dpo_lora_checkpoint/
#   ├── adapter_config.json         # LoRA 配置
#   ├── adapter_model.safetensors   # DPO 训练后的 LoRA 权重（在 SFT adapter 基础上更新）
#   └── trainer_state.json          # 训练状态
```

---

## 单卡 RTX 5090 为什么适合 QLoRA

RTX 5090 的规格（预估/当前信息）：
...
| 显存 | 32GB | 80GB | 80GB |
| 全量微调 8B | 困难（需要 ≥40GB） | 轻松 | 轻松 |
| QLoRA 微调 8B | 完美 | 轻松 | 轻松 |
| QLoRA 微调 70B | 困难 | 可行 | 轻松 |

### 面试话术

> "RTX 5090 32GB 是 QLoRA 的'甜点'卡。8B 模型 4bit 量化后约 4-5GB，加上 ~5M LoRA 参数、优化器状态、激活值，总计约 12-15GB，远低于 32G 上限。这意味着可以用更大的 batch size 和更长的序列长度，而不用像 24GB 卡那样精打细算。同时 5090 的 BF16 Tensor Core 性能远超上一代消费卡，反量化计算开销几乎可忽略。4090 到 5090 的最大变化是显存从 24G 升到 32G，这多出来的 8G 刚好让 8B 模型的 QLoRA 微调从'勉强能跑'变成'运行舒适区'。"
...
> "不是。rank 增大意味着更多的可训练参数和更强的表达能力，但也更容易过拟合小数据集。同时更大的 rank 会让 ΔW 的实际秩增加，但预训练权重的 fine-tuning 更新本身是低秩的——rank 超过'真正的本征维度'后，额外容量带来的是噪声而非信号。在医学项目中，r=16 配合 10K 条 SFT 数据取得了良好效果。实践中我建议从 r=8 起步，根据验证集指标逐步增大，通常在 r=32 到 r=64 之间会看到收益饱和。"

### Q2: "为什么不直接全量微调，要用 LoRA？"
...
### Q4: "merge_and_unload 后模型的推理速度会变快吗？"
...
### Q5: "如果 Qwen3-8B 用 rank=16 LoRA，哪些层的参数量最大？"
