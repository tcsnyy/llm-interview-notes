...
### 面试官可能怎么问
"画出 Transformer 的结构图，并解释每个组件的功能。"

### 这个问题考察什么
对 Transformer 架构的完整理解。
...
### 面试 3 分钟版本
在 1 分钟基础上展开每个组件的作用，特别是 causal mask 和 why decoder-only。

### 和我的项目怎么结合
- MiniMind 中从零实现了标准 Transformer（transformer.py），包括 MultiHeadAttention、PositionalEncoding、FFN 等
...
### 面试官可能怎么问
"Bert 和 GPT 的架构区别是什么？为什么现在大模型都用 decoder-only？"
...
### 面试官可能怎么问
"手写一下 Multi-Head Attention 的 forward 过程，包括维度变换。"
...
**现在模型普遍用 GQA**：Qwen3、Llama 3、Mistral 等都使用 GQA。

### 和我的项目怎么结合
MiniMind 项目中 `num_key_value_heads=2`（Q 头=8），使用了 GQA。
...
### 面试官可能怎么问
"causal mask 和 padding mask 有什么区别？为什么要区分？"
...
### 面试官可能怎么问
"Transformer 的 FFN 做了什么？SwiGLU 是什么？"
...
### 面试官可能怎么问
"为什么需要 Residual Connection？PreNorm 和 PostNorm 哪个好？"
...
### 和我的项目怎么结合
- Qwen3 使用 RoPE
- MiniMind 配置中 `rope_theta=1,000,000`，且支持 YaRN 外推
...
### 和我的项目怎么结合
- vLLM 部署使用 PagedAttention 管理 KV Cache
- 压测时 TTFT 由 prefill 决定，TPOT 由 decode + KV Cache 决定
...
11. 长上下文难在 Attention O(n²)、KV Cache 显存、位置外推、Lost in Middle
