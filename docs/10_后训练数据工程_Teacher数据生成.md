...
│  │                                                              │   │
│  │  基座模型: Qwen3-8B                                          │   │
│  │  训练数据: 55,000 条 Teacher+Judge 过滤的 SFT 数据            │   │
│  │         │                                                    │   │
│  │         ▼                                                    │   │
...
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│        SFT (55,000)      DPO Prompt Pool (13,000)                   │
│        ┌────────┐           ┌────────────┐                          │
│        │ 不重叠  │◄─────────►│  不重叠    │                          │
│        │ eval    │  独立集   │  DPO eval  │                          │
│        │ (2,000) │           │   (2,000)  │                          │
...
| Teacher 生成 | 85,000 | Teacher（DeepSeek-v4-pro）根据 prompt 生成的医学回答 | 医 Q+A 对 (prompt + Teacher 答案) |
| Judge 过滤后 | 55,000 | 通过严格筛选的高质量 SFT 数据 |
| 过滤通过率 | ~64.7% (55,000/85,000) | 但因去除的也包含原始 140K 中质量不够的部分（重复等），实际用 55K 中的子集 |
...
通过率说明：57,000 条先生成，经过去重和基础清洗后约 15,473 条进入 Judge 评分环节，55,000 条通过（64.7%），通过率并不低。
...
### Q: "你们的 55K 数据过 Judge 只剩 55,000 条，通过率是不是太低了？"

1. 更准确的计算：57,000 条先生成，经过去重和基础清洗后约 15,473 条进入 Judge 评分环节，55,000 条通过（64.7%），通过率并不低。
2. 质量优先于数量：LIMA 论文证明 1K 高质量 > 50K 低质量。宁愿用 11K 高质量数据也不要 50K 含噪数据在医学场景中带来安全隐患。
3. 通过率本身就是质量信号：低通过率说明 Judge 在真正发挥作用，不是走形式的。
4. 55,000 条对 8B 模型已经足够。
...
**我的项目实例如下**：HuatuoGPT2 142K 原始数据 → 按 55K/2K/13K/2K 四片划分 → DeepSeek-v4-pro 作为 teacher 生成 85K 条 → Judge 严格过滤(correctness=safety=hallucination=5, weighted≥4.8) → 55,000 条高质量 SFT 数据（64.7% 保留率）。
...
【数据划分】
  SFT train:    55,000  → Teacher 生成 → Judge 过滤 → 55,000 条
  SFT val:       2,000  → 独立验证
...
- [ ] 最终 SFT 多少条？55,000（不是 57K 也不是 140K）
...
**"55,000 条对一个 8B 模型是不是少了？"** → 不。医学 QA 是窄领域，11K 高质量数据对 8B 模型来说足够做有效的 SFT。关键在于每条数据的质量而非数量。而且 9,841 对 DPO 进一步做了偏好对齐。
