...
### Q: Reward shaping 是什么？为什么有时要给中间过程 reward？star:3

Reward shaping 是对 reward 函数做"塑形"——不只给最终答案 reward，还给中间推理步骤、工具调用过程等中间状态 bonus。

**为什么需要**：复杂任务（数学、代码、多轮 Agent 调用）的最终 answer reward 太稀疏——模型可能要尝试几千步才能碰巧走到正确答案。中间 reward 提供"路标"：
- 推理步骤的格式正确 → +0.1
- 调用了正确的工具 → +0.2
- 推理到一半就矛盾了 → -0.5（提前终止）

**风险**：中间 reward 设计不好会引入偏差——模型学会"刷中间分"而非"最终做对"。需要平衡 intermediate reward 和 final reward 的权重。

### Q: helpfulness 和 safety 冲突时，reward 如何设计？star:5

医学场景典型冲突：用户问"感冒吃什么药能快点好"，helpfulness 要求给具体建议（"可考虑对乙酰氨基酚缓解症状"），safety 要求不能给用药建议（"请咨询医生"）。

**设计策略——安全一票否决 + 分级回答**：
- 低风险（普通感冒科普）→ helpfulness 权重高，可以给通用建议 + 免责声明
- 中风险（特定药物询问）→ safety 优先，拒绝个体化建议但可给药品科普
- 高风险（儿童/孕妇/合并用药/剂量）→ safety 一票否决，必须拒绝 + 就医建议

**Reward 设计上**：如果 safety 分 < 阈值（如 <3/5），总 reward = safety 分 × 0.1（大幅惩罚）。如果 safety 分达标，用 helpfulness + correctness 加权求和。

**关键**：不能把所有药物问题一刀切打低分——"感冒药不能和酒一起喝"既安全又有用，应该给高分。

### Q: 如果 reward model 本身不可靠，会对 RLHF 训练造成什么影响？star:4

Reward model 不可靠 → 策略优化的梯度方向错误 → 模型学到 reward model 的偏好而非人类真正的偏好。

**具体表现**：
1. **长度偏置**：RM 偏爱长回答 → 模型越练越啰嗦
2. **风格偏置**：RM 喜欢固定格式（"首先...其次...最后"）→ 回答僵化
3. **知识盲区**：RM 判断不了医学正确性 → 编造内容也能得高分
4. **不一致**：同一回答两次打分差异大 → 训练信号噪声大

**缓解**：
1. 多 RM ensemble → 降低单模型偏差
2. LLM-as-Judge 交叉验证 → 用不同 judge 模型互校
3. 人工抽检 RM 打分 → 校准评分标准
4. 在训练中监控 reward 分布 → 发现异常及时停止

**我的项目做法**：使用 MiMo-v2.5-pro 的 multi-dimensional scoring（8维度）而非单一 reward score，且配合 pairwise 和 safety review 交叉验证评测结果，不依赖单一 reward 信号。
