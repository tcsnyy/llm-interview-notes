...
### Q: SFT 和 RL 的核心区别是什么？为什么 SFT 更像模仿学习，RL 更像基于反馈优化？star:5

SFT 模仿教师分布：给定 (prompt, answer)，最大化 $P(answer|prompt)$。本质上是在克隆教师行为，教师写什么模型学什么——模型看到的只有"正确答案"，没有"错误对比"。

RL（PPO/GRPO/DPO）引入偏好信号：不只告诉模型"什么是好的"，还告诉模型"什么是不好的"。DPO 用 chosen/rejected 对比，GRPO 用组内相对奖励排序。这让模型能在好坏之间建立梯度方向，而不是简单复制。

关键区别：SFT 的优化目标是**对数似然最大化**（越像教师越好），RL 的优化目标是**期望奖励最大化**（越好越被奖励）。SFT 学的是"教师怎么做"，RL 学的是"怎么做才能被奖励"。

### Q: 为什么大模型后训练通常先 SFT 再 RL？没有 SFT 直接 RL 会发生什么？star:5

1. **SFT 提供 cold start**：先让模型学会基本的指令跟随和对话格式。如果 base model 直接进入 RL，探索空间巨大，模型连"回答格式"都不懂，reward 信号完全无效。
2. **RL 对格式敏感**：PPO/GRPO 需要模型生成的回答能被 reward model 或 verifier 正确解析。如果格式混乱（如不会用 chat template），奖励无法分配。
3. **训练稳定性**：SFT 后的模型分布已经靠近"合理回答区域"，RL 只需微调偏好方向。直接从 base model 开始 RL，策略梯度方差极大，训练极不稳定。
4. **探索成本**：没有 SFT 的 base model 回答质量极低，RL 需要大量采样才能碰巧生成一个"好回答"获得正 reward，采样效率极低。

能不能跳过 SFT？理论上可以（比如用规则 reward 强约束格式），但实践中几乎没有成功的先例。SFT cold start 是后训练 pipeline 的标配。

### Q: 只做 SFT 不做 RL 是否可行？什么场景够用，什么场景必须 RL？star:4

**SFT 够用的场景**：简单指令跟随、风格转换、格式统一、领域知识注入（如医学 SFT）。

**必须引入 RL/DPO 的场景**：
1. 多目标平衡（helpfulness vs safety 需要偏好信号来决定取舍）
2. 防止过度拒答（SFT 拒答数据多了模型会过度保守，需要 DPO 的负例信号）
3. 复杂推理（数学/代码的中间步骤正确性需要 verifiable reward）
4. LLM-as-Judge 反馈优化（需要 RL 闭环迭代）

SFT 学"怎么做"，RL 学"怎么做更好"——两者互补而非替代。


### Q: Prompt few-shot 和 SFT 都能提升模型效果，它们的区别是什么？star:4

Few-shot 是在推理时在 prompt 里放几个示例，模型参数不变——利用的是 in-context learning 能力。SFT 是通过训练更新模型参数。
...
| 示例数量 | 1-10个 | 几百~几万条 |
| 泛化性 | 仅当前对话 | 跨对话持久化 |
| 适合场景 | 快速适配新任务、临时格式控制 | 持久化能力注入、大规模任务 |

**面试金句**："Few-shot 是推理时引导——给模型看几个例子让它现学；SFT 是参数更新——把例子刻进权重里。两者互补：SFT 做领域适配，Few-shot 做任务描述。"
